...
2.  **📋 Bi-Directional `task.md` Sync:** Automatically synchronizes planning steps into a live `task.md` file located securely in a `.focus/` subdirectory. Manually checking off boxes (`- [x]`) in your editor is watched in real-time, programmatically updating agent states in-memory.
3.  **🛡️ Gated Drift Blocking:** Hooks into tool executions. The **Planning Gate** gently warns and allows the LLM to write exploratory code (requiring a summary) before a plan is approved, preventing hard blocks. The **Execution Gate** blocks the worker if it attempts to edit an unauthorized path, prompting you: *"Approve file whitelisting? (y/n)"*. If blocked, it offers a quick re-planning pivot.
4.  **📊 Tools Optimizer (Token Pruning):** Prunes active tool schemas inside prompt context based on task intent, loading **zero tools** for pure explanations to save up to 80% context window tokens.
...
2. **Drafting:** The LLM writes the plan to a local `.focus/task.md` file.
...
| Agent | Behavior |
|---|---|
| **planner** | **Analyst first.** Reads the codebase before drafting. Never guesses; uses `focus_decision` for unclear requirements. Allowed to write exploratory code, but must summarize all activities and file changes. |
...
### 🪓 Dynamic Tools Optimizer (Token Pruning)

The `focus-tools-optimizer` acts as an intent classifier and token-saving firewall. It intercepts chat messages and aggressively hides tools the AI doesn't need for that specific request, drastically shrinking the context window footprint.

**Intent Buckets:** When a message is received in `IDLE` mode, it is scored against regex signals:
*   📝 **`write`:** Triggered by words like *"fix, change, bug"*. Loads modifying tools. Automatically whitelists any custom tool containing `write, edit, update, create, patch, etc.` in its name.
*   👀 **`read`:** Triggered by words like *"read, show, inspect"*. Removes modifying tools, allows `read_file`. Automatically whitelists any custom tool containing `search, read, query, get, fetch, etc.` in its name.
*   🔍 **`search`:** Triggered by words like *"find, grep"*. Allows read/search tools. (Shares auto-whitelist logic with `read`).
*   💬 **`respond`:** Triggered by short messages or words like *"explain, compare"*. Loads **ZERO tools**, saving thousands of tokens.

Whenever the tools optimizer activates, it explicitly lists the exact tool names it enabled in a UI notification so you always know what the agent can do.

**Focus-Mode Integration:** If `focus-mode` is actively working on a task (state is `planning`, `executing`, or `reviewing`), the `focus-tools-optimizer` automatically backs off and enables **ALL** tools. It only actively prunes tools when the system is sitting in the `IDLE` state.
